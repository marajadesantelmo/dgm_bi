...
#Gastos
cursor.execute("""
SELECT a.FechaCreacion, ai.Importe1, cc.Descripcion AS Concepto, cc.Numero, a.Descripcion AS Descripcion
FROM asientos a
...
cargas_sociales_mensual = cargas_sociales.groupby(['Numero', 'Descripcion', 'Mes'])['Importe1'].sum().reset_index()
cargas_sociales_mensual.columns = ['Numero', 'Descripcion', 'Mes', 'Cargas Sociales Total']
...
cargas_sociales_final = cargas_sociales_mensual_unegocio[['Unidad de Negocios', 'Mes', 'Concepto', 'Descripcion', 'Numero', 'Importe']]
...
sindicato_mensual = sindicato.groupby(['Numero', 'Descripcion', 'Mes'])["Importe1"].sum().reset_index()
sindicato_mensual.columns = ['Numero', 'Descripcion', 'Mes', 'Sindicato Total']
...
sindicato_final = sindicato_mensual_unegocio[['Unidad de Negocios', 'Mes', 'Concepto', 'Descripcion', 'Numero', 'Importe']]

egresos = gastos[gastos['Numero'].isin(['42201010', '42101010', '42101056', '42201031', '42101001', '42201001', '11504001', '11501001', '42201041' ])]


egresos['Mes'] = egresos["FechaCreacion"].dt.to_period("M")
egresos_mensual = egresos.groupby(["Mes", "Numero", "Descripcion", "Concepto"])["Importe1"].sum().reset_index()
...
egresos_mensual.columns = ['Mes', 'Numero', 'Descripcion', 'Concepto', 'Importe', 'Unidad de Negocios']
egresos_mensual = egresos_mensual[egresos_mensual['Unidad de Negocios'].isin(['Bs.As.', 'Salta'])]
datos = pd.concat([ventas_mensual, sueldos_mensual, cargas_sociales_final, sindicato_final, egresos_mensual])
datos['Descripcion'] = datos['Descripcion'].fillna("-")

datos.to_excel('margen_2026.xlsx')
